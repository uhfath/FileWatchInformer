...
namespace FileWatchInformer.Services.Emails
{
    internal class EmailSenderService : IDisposable, IAsyncDisposable
    {
        ...
        public async Task SendEmailAsync(string address, string subject, EmailInfo emailInfo, CancellationToken cancellationToken = default)
        {
            ...
            {
                body = MessageUtils.PrepareMessage(emailInfo.UserName, emailInfo.RootFolder, emailInfo.UserFolder, _emailConfig.DefaultBody, emailInfo.Files);
            }
